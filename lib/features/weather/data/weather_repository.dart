...
class WeatherRepositoryImpl implements WeatherRepository {
  ...
  WeatherRepositoryImpl() : _service = WeatherService();
  ...
  Future<WeatherData> getCurrentWeather(String city) async {
    try {
      ...
      throw WeatherFailure('Не удалось получить текущую погоду: ${e.toString()}');
    }
  }
  ...
  Future<WeatherData> getWeatherForecast(String city) async {
    try {
      ...
      final daily = (fore['list'] as List).where((item) {
      ...
      throw WeatherFailure('Не удалось получить прогноз погоды: ${e.toString()}');
    }
  }
